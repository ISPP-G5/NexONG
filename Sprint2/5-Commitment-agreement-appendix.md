...
Al finalizar el Sprint se calculará el total de horas realizadas por ese miembro del equipo, en donde si se encuentra dentro del rango de horas estimado, la primera división tendrá un valor de 1, multiplicándose por la puntuación del poker planning. La formula para hacer el calculo sería:

$$\left(E_{total} = \sum_{i=0}^{n_{tareas} } \left( \frac{E_{realizado_i}}{E_{estimado_i}} \times Puntuación_{poker_i} \right)\right)$$
